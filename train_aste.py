...
class ASTETrainer:
    ...
    def initialize_models(self):
        ...
        logger.info(f"BASELINE: Using exact baseline Stage 1 LR={self.learning_rate} as specified")
        ...
        stage_two_lr = self.learning_rate
        ...
        logger.info(f"Stage Two learning rate: {stage_two_lr} (same as Stage One: {self.learning_rate}) - Baseline")
        ...
        logger.info("Using baseline loss functions (no class weights, equal task weights)")
    ...
    def train_stage_one(self):
        ...
        for epoch in range(self.start_epoch if self.stage == 1 else 0, self.num_epochs):
            ...
            if (epoch + 1) % self.eval_interval == 0:
                ...
                if self.epochs_without_improvement >= self.patience:
                    logger.info(f"No improvement for {self.epochs_without_improvement} epochs (continuing as per baseline)")
        ...
        if self.best_val_f1 == 0.0:
            logger.warning(" Best F1 not improved, saving current model as fallback...")
            self.save_model(self.stage_one_model, f'{self.args.dataset}_stage_one_best.pt')
    ...
    def calculate_stage_one_loss(self, outputs, labels, target_labels, opinion_labels, attention_mask):
        ...
        active_tg = opinion_labels.view(-1)[active_positions]
    ...
    def generate_stage_one_prediction_pairs(self, data_loader, use_ground_truth_for_labels=True):
        ...
        total_valid = 0
        total_invalid = 0
        samples_with_aspects = 0
        samples_with_opinions = 0
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(data_loader, desc="Stage 1 predictions")):
                ...
                for i in range(batch_size):
                    ...
                    if pred_aspects:
                        samples_with_aspects += 1
                    if pred_opinions:
                        samples_with_opinions += 1
        ...
        logger.info(f"  Samples with predicted aspects: {samples_with_aspects}")
        logger.info(f"  Samples with predicted opinions: {samples_with_opinions}")
    ...
    def print_final_results_summary(self, final_results, stage_one_results):
        ...
        baseline_stage_one_targets = {
            '14res': 71.95, '14lap': 62.34, '15res': 65.79, '16res': 71.73
        }
        baseline_opinion_targets = {
            '14res': 82.45, '14lap': 74.84, '15res': 78.02, '16res': 83.73
        }
        
        if self.args.dataset in baseline_stage_one_targets:
            baseline_aspect_f1 = baseline_stage_one_targets[self.args.dataset]
            baseline_opinion_f1 = baseline_opinion_targets[self.args.dataset]
            
            logger.info(f"   {self.args.dataset} Baseline Aspect Target:  {baseline_aspect_f1:.1f}%")
            aspect_diff = (stage_one_results['f1']*100) - baseline_aspect_f1
            aspect_status = "" if aspect_diff >= -2.0 else ""
            logger.info(f"   {self.args.dataset} Aspect Difference:    {aspect_diff:+.1f}% ({aspect_status})")
            
            logger.info(f"   {self.args.dataset} Baseline Opinion Target: {baseline_opinion_f1:.1f}%")
            opinion_diff = (stage_one_results['opinion_f1']*100) - baseline_opinion_f1
            opinion_status = "" if opinion_diff >= -2.0 else ""
        ...
        baseline_final_targets = {
            '14res': 51.89, '14lap': 43.50, '15res': 46.79, '16res': 53.62
        }
        if self.args.dataset in baseline_final_targets:
            baseline_f1 = baseline_final_targets[self.args.dataset]
            logger.info(f"   {self.args.dataset} Baseline Final Target:   {baseline_f1:.1f}%")
            final_diff = (final_results['triplet_f1']*100) - baseline_f1
            final_status = " ABOVE" if final_diff >= 0 else "  BELOW"
    ...
    def optimize_stage_two_threshold(self):
        ...
        if len(dev_pairs) == 0:
            logger.warning(" No dev pairs for threshold optimization, using default 0.5")
            return 0.5
        ...
        if len(all_probs) == 0:
            logger.warning(" No pairs found for threshold optimization, keeping default 0.5")
            return 0.5
        best_score = 0
        best_threshold = 0.5
    ...
    def generate_stage_one_based_pairs(self, data_loader, use_ground_truth_labels=True):
        logger.warning(" generate_stage_one_based_pairs() is deprecated, using generate_stage_one_prediction_pairs()")
    ...
    def generate_ground_truth_pairs(self, data_loader):
        logger.info(" BASELINE METHOD: Generating pairs from ground truth annotations...")
        pairs = []
        total_valid = 0
        total_invalid = 0
...
class ASTEDataset(Dataset):
    def __init__(self, examples, word_to_id, label_to_id, target_to_id):
        ...
        import spacy
        self.nlp = spacy.load("en_core_web_lg")  # Reverted from lg to sm (baseline config)
...
def main():
    parser = argparse.ArgumentParser(description='ASTE Training')
    ...
    parser.add_argument('--milestone_interval', type=int, default=30)
    parser.add_argument('--resume_from_checkpoint', type=str, default=None)
    
    args = parser.parse_args()
    
    logger.info("ASTE Training - Baseline Implementation")
